...
const HomeScreen: React.FC = () => {
  ...
  const handleQuickAction = (action: string) => {
    switch (action) {
      case 'qr':
        Alert.alert('QR Scan', 'QR scanner would open here');
        break;
      case 'schedule':
        setActiveTab('schedule');
        break;
      case 'tracking':
        navigation.navigate('BusTracking', { screen: 'live-tracking' });
        break;
      case 'sos':
        Alert.alert('Emergency SOS', 'Emergency alert sent with your location!');
        break;
      case 'child':
        Alert.alert('Child Mode', 'Child tracking mode activated');
        break;
      case 'offline':
        navigation.navigate('BusTracking', { screen: 'offline-map' });
        break;
      case 'alerts':
        Alert.alert('Alerts', 'Your notification alerts');
        break;
      case 'favorites':
  ...
  const handleSearch = () => {
    ...
    const mockSearchResults = [
      {
        ...
        ticketPrice: '₹45',
        status: 'on-time',
        currentLocation: searchData.from + ' Station',
        speed: 35,
        passengerLoad: 65,
    ...
    navigation.navigate('BusTracking', {
      screen: 'bus-details',
      params: { buses: mockSearchResults }
    });
  };
  ...
  const handleCurrentBusPress = () => {
    navigation.navigate('BusTracking', {
      screen: 'bus-details',
      params: { 
        buses: currentBus ? [currentBus] : nearbyBuses.length > 0 ? nearbyBuses : [
          {
            id: '1',
            route: 'Route 101',
            busNumber: 'KA-01-AB-1234',
            ...
            <View style={styles.tabContent}>
              ...
              <View style={styles.recentSearches}>
                <TouchableOpacity style={styles.recentSearchItem} onPress={() => {
                  setSearchData({ from: 'Downtown', to: 'University' });
                ...
                <TouchableOpacity style={styles.recentSearchItem} onPress={() => {
                  setSearchData({ from: 'Mall', to: 'Airport' });
                  setTimeout(handleSearch, 100);
                }}>
                  <Ionicons name="time-outline" size={16} color="#6E473B" />
                ...
                <TouchableOpacity style={styles.recentSearchItem} onPress={() => {
                  setSearchData({ from: 'Station', to: 'Hospital' });
                  setTimeout(handleSearch, 100);
                }}>
                  <Ionicons name="time-outline" size={16} color="#6E473B" />
            ...
            <View style={styles.tabContent}>
              ...
              {nearbyBuses.length > 0 ? (
                nearbyBuses.slice(0, 3).map((bus) => (
                  <TouchableOpacity key={bus.id} style={styles.busItem} onPress={() => {
                    navigation.navigate('BusTracking', {
                      screen: 'bus-details',
                      params: { buses: [bus] }
                    });
        ...
        <View style={styles.currentBusSection}>
          ...
          <TouchableOpacity style={styles.currentBusCard} onPress={handleCurrentBusPress}>
            <View style={styles.busHeader}>
              <Text style={styles.busRouteLarge}>Route 101</Text>
              <View style={[
                styles.busStatusBadge,
                styles.busMoving
              ]}>
                <Text style={styles.busStatusText}>Moving</Text>
              </View>
            </View>
            <Text style={styles.busLocationLarge}>
              Current: {isLoadingLocation ? 'Getting location...' : currentLocation}
            </Text>
            <Text style={styles.busNextStop}>Next Stop: University • 5 min</Text>
